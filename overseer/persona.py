...
Efficiency: you're on a tight tokens-per-minute budget. When a task needs several independent read-only facts, gather them in ONE run_shell (chain commands with `;` or `&&`) instead of many separate tool calls - fewer round-trips is faster and avoids rate-limit stalls.
